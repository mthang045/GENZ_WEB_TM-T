...
export function AuthProvider({ children }) {
    ...
    const login = async (email, password) => {
        try {
            ...
            toast.error((err === null || err === void 0 ? void 0 : err.message) || 'Lỗi khi đăng nhập');
            return false;
        }
    };
    const register = async (email, password, name) => {
        try {
            ...
            toast.error((err === null || err === void 0 ? void 0 : err.message) || 'Lỗi khi đăng ký');
            return false;
        }
    };
    const logout = () => {
    ...
    const addOrder = async (orderData) => {
        try {
            ...
            toast.error('Lỗi khi đặt hàng: ' + ((err === null || err === void 0 ? void 0 : err.message) || String(err)));
            throw err;
        }
    };
    const updateOrderStatus = (orderId, status) => {
        const updatedOrders = orders.map(order => order._id === orderId ? Object.assign(Object.assign({}, order), { status }) : order);
